...
let productVm = ProductVM()
    ...
    override func viewDidLoad() {
        ...
        priceLabel.text = "\(product?.price ?? 0000)" + "   OMR".localizede
...
hideDescriptionLa()
    ...
    @objc func addToCart(){
        productVm.AddToCart(productId: product?.id ?? "", quantity: numberOfUnit, customer: true, vc: self)
    }
    ...
    func subscribeToAddToCartButton(){
        addProduct.rx.tap.throttle(RxTimeInterval.microseconds(5000),scheduler: MainScheduler.instance)
            ...
            guard let self = self else {return}
                ...
                self.productVm.AddToCart(productId: self.product?.id ?? "", quantity: self.numberOfUnit ?? 0, customer: true, vc: self)
    ...
    func subscribeToFavoriteButton(){
        favButton.rx.tap.throttle(RxTimeInterval.microseconds(5000), scheduler: MainScheduler.instance)
            .subscribe {[weak self] (_) in
                ...
                self.productVm.MakeFav(id: self.product?.id ?? "", vc: self)

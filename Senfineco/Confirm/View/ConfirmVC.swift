        ...
        conf.loginModelObservable.subscribe { (data) in
            if self.kind == "customer"{
                ...
                if data.element?.status == true {
                    ...
                    HelperK.setUserData(phone: data.element?.payload?.phone ?? "", lname: data.element?.payload?.lname ?? "", fname: data.element?.payload?.fname ?? "", email: data.element?.payload?.email ?? "", type: data.element?.payload?.role ?? "")
